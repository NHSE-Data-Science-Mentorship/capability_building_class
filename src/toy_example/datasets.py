...
class FoundrySQLWrapper():
    ...
    def get_ecds(self, location_ids):
        ...
        CASE
            WHEN (discharge_destination_snomed_ct
            IN {list_to_sql_array(ADMITTED_DISCHARGE_DESTINATION)}) THEN 1
            ELSE 0
        END AS admitted
        FROM "/NHS/NHS: ECDS Patient Level/data/transform/ecds_v2"
        WHERE (der_provider_code IN {list_to_sql_array(location_ids)})
        AND (discharge_destination_snomed_ct IS NOT NULL)
        AND (DATE(arrival_time)>=DATE('2022-03-01'))
        AND DATE(arrival_time)<=DATE('2022-03-02')
        """
